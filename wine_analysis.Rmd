...
```{r}
articles_w_topics20 = as.data.frame(cbind(winedata[rowTotals> 0, ], (topic_probabilities20$topics)))
# Rename a column in R
```
...
summary(lm(points ~ log(price) + taster_twitter_handle + vintage_year_cat + topic_1 + topic_2 + topic_3 + topic_4 + topic_5 + topic_6 + topic_7 + topic_8 + topic_9 + topic_10 + topic_11 + topic_12 + topic_13 + topic_14 + topic_15 + topic_16 + topic_17 + topic_18 + topic_19 + topic_20, articles_w_topics20))

# Run regsubsets for model selection
subsets = regsubsets(points ~ log(price) + taster_twitter_handle + vintage_year_cat + topic_1 + topic_2 + topic_3 + topic_4 + topic_5 + topic_6 + topic_7 + topic_8 + topic_9 + topic_10 + topic_11 + topic_12 + topic_13 + topic_14 + topic_15 + topic_16 + topic_17 + topic_18 + topic_19 + topic_20, data=articles_w_topics20, nvmax=50,method="backward")
